...
async function rockPaperScissors(interaction){
    ...
    collector.on('collect', async (collected) => {
        ...
        const alreadyInArray = choiceArray.some(obj => obj.userID === collected.user.id);

        if (userXP.xp >= tokens && alreadyInArray === false && choiceArray.length < 2){
            const choice = {
            ...
            await collected.reply({
                ...
                await collected.reply({
                    content: "You either don't have enough tokens for this or you've already made a choice!",
                    ephemeral: true
            });
        }
        ...
        if (choiceArray.length === 2){
            ...
            switch(choiceArray[0].rps) {
                ...
                if (winner == 'tie'){
                    ...
                    await interaction.channel.send({
                        content: `${winnerMember.user} chose ${winner.rps} and ${loserMember.user} chose ${loser.rps}.\n${winnerMember.user} wins **${tokens * 2 * 0.95} ReiGn Tokens**!`
                    });
